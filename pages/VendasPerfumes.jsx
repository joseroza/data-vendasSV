...
function calcular(row) {
  ...
  return { custo_reais: custo, lucro_bruto: lucroBruto, lucro_liquido: lucroBruto, valor_faltante: faltante };
}
...
export default function VendasPerfumes() {
  ...
  const handleSaveNew = () => {
    ...
    createMutation.mutate({ ...newRow, ...calcular(newRow) });
  };
  ...
  const handleSaveEdit = () => {
    updateMutation.mutate({ id: editingId, data: { ...editData, ...calcular(editData) } });
  };
  ...
  // CORREÇÃO: totais de lucro separados (bruto e líquido)
  const totalLucroBruto = filteredVendas.reduce((acc, v) => acc + (v.lucro_bruto || 0), 0);
  const totalLucroLiquido = filteredVendas.reduce((acc, v) => acc + (v.lucro_liquido || 0), 0);

  // CORREÇÃO: valor pendente real — se valor_faltante não preenchido, usa preco_venda completo
  const totalPendente = filteredVendas.reduce((acc, v) => {
    if (v.status_pagamento === 'PAGO') return acc;
    const faltante = (v.valor_faltante > 0) ? v.valor_faltante : (v.preco_venda || 0);
    return acc + faltante;
  }, 0);

  // CORREÇÃO: marcas carregadas dinamicamente do estoque
  const marcasUnicas = [...new Set(estoque.map(p => p.marca).filter(Boolean))].sort();
      ...
      <div className="max-w-full mx-auto space-y-4">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Buscar por comprador, perfume ou marca..."
              value={searchTerm}
        ...
        <div className="bg-white/90 rounded-2xl border border-gray-200 shadow-lg overflow-x-auto">
          <table className="w-full text-sm border-collapse">
              ...
              {newRow && (
                <tr className="bg-purple-50/60 border-b border-purple-100">
                  ...
                  <td className="px-2 py-1">
                    <input
                      list="perfume-options-new"
                      value={newRow.nome_perfume}
                      onChange={e => handlePerfumeSelectNew(e.target.value)}
                      placeholder="Perfume*"
                      className="w-40 px-2 py-1 text-sm border border-transparent rounded focus:border-purple-400 focus:outline-none focus:bg-white bg-transparent hover:bg-white/60"
                    />
                    <datalist id="perfume-options-new">
                  ...
                  <td className="px-2 py-1">
                    <select
                      value={newRow.marca || ""}
                      onChange={e => updateNewRow('marca', e.target.value)}
                      className="w-32 px-2 py-1 text-sm border border-transparent rounded focus:border-purple-400 focus:outline-none bg-transparent hover:bg-white/60 focus:bg-white"
                    >
                      <option value="">Marca</option>
                      {marcasUnicas.map(m => <option key={m} value={m}>{m}</option>)}
                    </select>
                  </td>
                  <td className="px-2 py-1"><EditCell type="number" value={newRow.preco_dolar} onChange={v => updateNewRow('preco_dolar', v)} width="w-20" /></td>
                  ...
                  <td className="px-2 py-1 bg-orange-50/30"><span className="px-2 text-orange-700">{newRow.custo_reais?.toFixed(2)}</span></td>
                  <td className="px-2 py-1"><EditCell type="number" value={newRow.preco_venda} onChange={v => updateNewRow('preco_venda', v)} width="w-24" /></td>
                  <td className="px-2 py-1 bg-green-50/30"><span className={`px-2 font-medium ${newRow.lucro_bruto >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{newRow.lucro_bruto?.toFixed(2)}</span></td>
                  <td className="px-2 py-1 bg-green-50/30"><span className={`px-2 font-medium ${newRow.lucro_liquido >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{newRow.lucro_liquido?.toFixed(2)}</span></td>
                  <td className="px-2 py-1">
                  ...
                  <td className="px-2 py-1"><EditCell type="number" value={newRow.parcela_1} onChange={v => updateNewRow('parcela_1', v)} width="w-20" /></td>
                  <td className="px-2 py-1"><EditCell type="number" value={newRow.parcela_2} onChange={v => updateNewRow('parcela_2', v)} width="w-20" /></td>
                  <td className="px-2 py-1"><EditCell type="number" value={newRow.parcela_3} onChange={v => updateNewRow('parcela_3', v)} width="w-20" /></td>
                  <td className="px-2 py-1 bg-amber-50/30"><span className={`px-2 font-medium ${newRow.valor_faltante > 0 ? 'text-amber-600' : 'text-gray-400'}`}>{newRow.valor_faltante > 0 ? newRow.valor_faltante?.toFixed(2) : "-"}</span></td>
                  <td className="px-2 py-1">
                    <div className="flex gap-1 justify-center">
                      ...
                      <button onClick={() => setNewRow(null)} className="p-1.5 rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200"><X className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              )}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 text-right bg-green-50/30">
                      <span className={`px-2 font-medium ${(isEditing ? row.lucro_bruto : venda.lucro_bruto) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                        {(isEditing ? row.lucro_bruto : venda.lucro_bruto)?.toFixed(2) || "-"}
                      </span>
                    </td>
                    <td className="px-2 py-1 text-right bg-green-50/30">
                      <span className={`px-2 font-medium ${(isEditing ? row.lucro_liquido : venda.lucro_liquido) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                        {(isEditing ? row.lucro_liquido : venda.lucro_liquido)?.toFixed(2) || "-"}
                      </span>
                    </td>
                    <td className="px-2 py-1">
                      {isEditing
                        ...
                        : <Badge className={`text-xs border ${statusColors[venda.status_pagamento]}`}>{venda.status_pagamento}</Badge>}
                    </td>
                    <td className="px-2 py-1">
                      {isEditing
                        ...
                        : <span className="px-2 text-gray-600 text-xs">{venda.forma_pagamento}</span>}
                    </td>
                    <td className="px-2 py-1">
                      {isEditing
                        ? <SelectCell value={row.plataforma} onChange={v => updateEditData('plataforma', v)} options={PLATAFORMAS} placeholder="Plataforma" width="w-36" />
                        : <span className="px-2 text-gray-500 text-xs">{venda.plataforma || "-"}</span>}
                    </td>
                    <td className="px-2 py-1 text-right">
                      {isEditing
                    ...
                    <td className="px-2 py-1 text-right bg-amber-50/30">
                      <span className={`px-2 font-medium ${(isEditing ? row.valor_faltante : venda.valor_faltante) > 0 ? 'text-amber-600' : 'text-gray-400'}`}>
                        {(isEditing ? row.valor_faltante : venda.valor_faltante) > 0
                          ? (isEditing ? row.valor_faltante : venda.valor_faltante)?.toFixed(2)
                          : "-"}
                      </span>
                    </td>
                    <td className="px-2 py-1">
            ...
            {/* CORRIGIDO: Lucro Bruto e Lucro Líquido com totais separados */}
            <tfoot>
              <tr className="bg-purple-50 border-t-2 border-purple-200 font-semibold">
                ...
                <td className="px-3 py-2 text-right text-emerald-600">{totalLucroBruto.toFixed(2)}</td>
                <td className="px-3 py-2 text-right text-emerald-600">{totalLucroLiquido.toFixed(2)}</td>
                <td colSpan={6} />
