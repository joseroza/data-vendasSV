...
function calcular(f) {
  ...
  return { custo_reais: custo, lucro_bruto: lucro, lucro_liquido: lucro, valor_faltante: faltante > 0 ? faltante : 0 };
}
...
export default function NovaVenda() {
  ...
  // CORRIGIDO: marcas e perfumes carregados dinamicamente do estoque
  ...
  // CORRIGIDO: carregadas do estoque, não hardcoded
  const marcasUnicas = [...new Set(estoque.map(p => p.marca).filter(Boolean))].sort();
  const perfumeOptions = estoque.map(p => p.perfume).filter(Boolean);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <AnimatePresence>
          {saved && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              className="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-emerald-700">
              <CheckCircle2 className="w-5 h-5" /> Venda registrada com sucesso!
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            {/* Bloco 1: Dados da Venda */}
            <div className="bg-white/90 rounded-2xl border border-gray-100 shadow p-6 space-y-4">
              <h2 className="font-semibold text-gray-700 text-sm uppercase tracking-wider border-b pb-2">Dados da Venda</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={e => handlePerfumeSelect(e.target.value)}
                    placeholder="Digite ou selecione"
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    required
                  />
                  <datalist id="perfume-list">
                    {perfumeOptions.map(p => <option key={p} value={p} />)}
                  </datalist>
                </div>
                <div className="space-y-1">
                  <Label>Marca</Label>
                  {/* CORRIGIDO: marcas do estoque, dinâmicas */}
            ...
            <div className="bg-white/90 rounded-2xl border border-gray-100 shadow p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={calc.valor_faltante.toFixed(2)} disabled
