    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div><Label>Produto Vendido *</Label><Input placeholder="Ex: iPhone 15 Pro, Samsung S24..." /></div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label>Preço de Custo (BRL) *</Label>
              <Input type="number" placeholder="0.00" value={precoCusto} onChange={e => setPrecoCusto(e.target.value)} />
            </div>
            <div>
              <Label>Preço de Venda (BRL) *</Label>
              <Input type="number" placeholder="0.00" value={precoVenda} onChange={e => setPrecoVenda(e.target.value)} />
            </div>
          </div>
          ...
          {custoNum > 0 && vendaNum > 0 && (
            <div className="p-3 rounded-lg bg-muted/50 border border-border/50 flex items-center gap-4">
              ...
              <span className="text-sm">Lucro: <span className={`font-semibold font-display ${lucro >= 0 ? "text-success" : "text-destructive"}`}>R$ {lucro.toFixed(2)}</span></span>
            </div>
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {parcelado && (
            <div className="space-y-4 p-4 rounded-lg border border-primary/20 bg-primary/5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label>Data da Venda *</Label><Input type="date" /></div>
                <div><Label>Quantidade de Parcelas *</Label><Input type="number" min={2} max={12} placeholder="Ex: 3" /></div>
              </div>
              <p className="text-xs text-muted-foreground">As datas das parcelas serão geradas automaticamente.</p>
            </div>
          )}
